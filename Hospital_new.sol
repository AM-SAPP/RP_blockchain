...
contract HospitalContract{
    ...
    function getHospitalDetail(address haddr) external view returns(ReturnFormatOfHospital memory){
        ReturnFormatOfHospital memory h;
    ...
    function getDetailsOfMultipleHospital(address [] memory hosp_addresses) external view returns(ReturnFormatOfHospital [] memory){

        uint256 counter = hosp_addresses.length;
        ReturnFormatOfHospital [] memory hosp_list = new ReturnFormatOfHospital[](counter);
