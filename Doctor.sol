...
contract DoctorContract{
    ...
    mapping(string => address) SearchDoctorWithRegistrationNumber;
    ...
    function registerDoctor(string memory _name , string memory _regNo,uint8 _age) external checkDoctorRegistered(msg.sender){
        ...
        countOfDoctors++;
        SearchDoctorWithRegistrationNumber[DoctorInfo[sender].registrationNumber] = sender;
    }

    function getDoctorWithRegistrationNumber(string memory registrationNumber) external view
    returns(MyStructs.ReturnFormatOfDoctor memory){
        address daddr = SearchDoctorWithRegistrationNumber[registrationNumber];
        MyStructs.ReturnFormatOfDoctor memory d;
        ...
        d.doctorAddress = DoctorInfo[daddr].owner;
        return d;
     }


    // This is used by patient
    function addPatientToAccessList(address daddr) external 
    checkDoctorRegistered(daddr) checkPatientNotInList(tx.origin){
        address paddr = tx.origin;
        DoctorInfo[daddr].patientAccessList.push(paddr);
        DoctorInfo[paddr].isPatient_in_accessList[paddr] = true;
    }


    // This is used by doctor
    ...
    function addHospitalToPendingList(address haddr) external
    checkHospitalInPendingList(haddr) {
        address daddr = tx.origin;
    ...
    returns (MyStructs.ReturnFormatOfDoctor [] memory) {
        ...
        MyStructs.ReturnFormatOfDoctor[] memory specializedDoctors = new MyStructs.ReturnFormatOfDoctor[](counter);
    ...
    view returns(MyStructs.ReturnFormatOfDoctor [] memory){
        uint256 counter = counterOfQualDoctor[qualificationCode];
        MyStructs.ReturnFormatOfDoctor[] memory qualifiedDoctors = new MyStructs.ReturnFormatOfDoctor[](counter);
        for (uint256 i=1;i<=counter;i++){
    ...
    function getDetailsOfMultipleDoctor(address [] memory doc_addresses) external view returns (MyStructs.ReturnFormatOfDoctor [] memory){
        ...
        MyStructs.ReturnFormatOfDoctor [] memory doc_list = new MyStructs.ReturnFormatOfDoctor[](counter);
