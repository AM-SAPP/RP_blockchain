...
contract PatientContract{
    ...
    struct Patient{
        ...
        uint8 weightInKg;
        uint8 heightInCm;
        mapping(uint256 => Report) MedicalHistory;
    ...
    function getPatientsWithSpecifiedBloodGroup(uint8 _bloodgroup) external view returns (ReturnFormatOfPatient [] memory){
        uint256 counter = countOfSpecializedBloodGroup[_bloodgroup];
        ReturnFormatOfPatient [] memory patientsWithBlood = new ReturnFormatOfPatient[](counter);  
        for(uint256 i=0;i<counter;i++){
           ReturnFormatOfPatient memory p;
    ...
    view returns(ReturnFormatOfPatient memory){
       ReturnFormatOfPatient memory p;
           p.name = PatientInfo[paddr].name;
           p.age =  PatientInfo[paddr].age;
           p.bloodgroup = bloodGroups[PatientInfo[paddr].bloodgroup];
    ...
    view returns (ReturnFormatOfPatient [] memory )
    {
        ...
        ReturnFormatOfPatient [] memory pat_list = new ReturnFormatOfPatient[](counter);
        for (uint256 i=0;i<counter;i++){
           ReturnFormatOfPatient memory p;
